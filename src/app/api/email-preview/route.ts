...
export async function GET(req: NextRequest) {
  ...
  if (!html) return new Response('unknown template', { status: 404 });

  const nav = Object.keys(templates).map(k =>
    `<a href="?t=${k}" style="display:inline-block;margin:4px;padding:6px 14px;background:${t === k ? '#3d1f99' : '#1e1040'};border:1px solid rgba(108,59,255,0.4);border-radius:8px;color:#C4A8FF;font-size:13px;text-decoration:none;font-family:monospace">${k}</a>`
  ).join('');
