  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KpiCard icon={<PhoneCall size={18} />} label="Llamadas (24h)" value={callList24h.length.toString()} color="#00e5ff" />
        <KpiCard icon={<TrendingUp size={18} />} label="Leads (24h)" value={todayLeads.toString()} color="#22c55e" />
          ...
          {criticalAgents.map(a => {
            ...
            return (
              <Link key={a.id} href={`/admin/agentes/${a.id}`}
                ...
                <span className="text-sm text-white flex-1">{a.business_name}</span>
            ...
            <div className="flex flex-col divide-y" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
              {(recentLeads ?? []).map(lead => (
                <Link key={lead.id} href={`/admin/agentes/${lead.agent_id}`}
                  className="py-2.5 flex items-start justify-between gap-3 hover:bg-white/[0.02] transition-colors -mx-1 px-1 rounded">
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-white truncate">{lead.nombre ?? 'Sin nombre'}</div>
                    {lead.negocio && (
                      <div className="text-xs truncate mt-0.5" style={{ color: 'rgba(255,255,255,0.4)' }}>{lead.negocio}</div>
                    )}
                    {lead.servicio && (
                      <div className="text-xs truncate mt-0.5" style={{ color: '#00e5ff' }}>{lead.servicio}</div>
                    )}
                  </div>
                  <div className="flex-shrink-0 text-right">
                    <div className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>{agentMap[lead.agent_id] ?? ''}</div>
            ...
            <div className="flex flex-col divide-y" style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
              {(recentCalls ?? []).map(call => (
                <Link key={call.id} href={`/admin/agentes/${call.agent_id}`}
                  className="py-2.5 flex items-center justify-between gap-3 hover:bg-white/[0.02] transition-colors -mx-1 px-1 rounded">
                  <div className="min-w-0">
                    <div className="text-sm text-white truncate">{call.caller_number || 'Desconocido'}</div>
                    <div className="text-xs mt-0.5 truncate" style={{ color: 'rgba(255,255,255,0.35)' }}>{agentMap[call.agent_id] ?? ''}</div>
                  </div>
  ...
  return (
    <div className="p-5 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xs font-semibold tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.3)' }}>{title}</h2>
        {link && (
          <Link href={link} className="text-xs hover:underline" style={{ color: '#00e5ff' }}>Ver todo →</Link>
        )}
      </div>
      {children}
    </div>
...
function Empty({ text }: { text: string }) {
  return <p className="text-xs py-6 text-center leading-relaxed" style={{ color: 'rgba(255,255,255,0.25)' }}>{text}</p>;
}
